...
import { Zipchaindata } from "@/data/product";
...
const Zipchains = () => {
  ...
  useEffect(() => {
    fetch("/api/products?category=zipchains&limit=9")
      .then((r) => r.json())
      .then((d) => {
        if (d.success && d.data.products.length > 0) {
          setProducts(d.data.products);
        } else {
          setProducts(Zipchaindata);
        }
      })
      .catch(() => setProducts(Zipchaindata))
      .finally(() => setLoading(false));
  }, []);

  const items = loading ? Zipchaindata : products;
