...
const AdminSidebar = () => {
  ...
  return (
    <div>
      <div className="m-4 cursor-pointer text-secondary hover:text-primary active:text-primary" onClick={() => setIsOpen(true)}>
        <MenuIcon />
      </div>

      <div className={`fixed inset-0 bg-heading/50 z-40 transition-opacity duration-300 ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
      ...
      <aside className={`fixed top-0 left-0 z-50 h-screen w-64 border-r border-border bg-card flex flex-col py-8 px-4 shadow-2xl transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <button onClick={() => setIsOpen(false)} className="absolute top-4 right-4 text-muted-text hover:text-primary">
          <CloseIcons />
        </button>

        <div className="px-4 py-3 mb-4">
          <h2 className="text-2xl font-semibold text-primary">{siteName ?? "Heritage Threads"}</h2>
